    ...
    switch (chartInstance->c2_is_c2_EV_Pwr_Manager) {
      ...
      if (c2_out) {
        ...
        *c2_Mode = 2U;
        ...
        c2_b_temp = (_SFD_CCP_CALL(8U, 0, (real_T)*c2_veh_spd < 0.05 * (real_T)
          *c2_veh_spd / 100.0 != 0U, chartInstance->c2_sfEvent) != 0);
        if (c2_b_temp) {
          c2_b_temp = (_SFD_CCP_CALL(8U, 1, (real_T)*c2_Accel_Cmd < 0.05 != 0U,
            chartInstance->c2_sfEvent) != 0);
        }
     ...
     case c2_IN_Coast:
      ...
      _SFD_CT_CALL(TRANSITION_BEFORE_PROCESSING_TAG, 3U,
                   chartInstance->c2_sfEvent);
      c2_d_temp = (_SFD_CCP_CALL(3U, 0, (real_T)*c2_Decel_Cmd > 0.25 * (real_T)
        *c2_veh_spd / 100.0 != 0U, chartInstance->c2_sfEvent) != 0);
      if (c2_d_temp) {
        c2_d_temp = (_SFD_CCP_CALL(3U, 1, *c2_veh_spd > 1.0F != 0U,
          chartInstance->c2_sfEvent) != 0);
      }

      c2_g_out = (CV_TRANSITION_EVAL(3U, (int32_T)c2_d_temp) != 0);
      if (c2_g_out) {
        if (_SFD_TRANSITION_CONFLICT_CHECK_ENABLED()) {
          unsigned int transitionList[3];
          unsigned int numTransitions = 1;
          transitionList[0] = 3;
          _SFD_TRANSITION_CONFLICT_CHECK_BEGIN();
          c2_h_out = ((real_T)*c2_Accel_Cmd > 0.25 * (real_T)*c2_veh_spd / 100.0);
          if (c2_h_out) {
            transitionList[numTransitions] = 6;
            numTransitions++;
          }

          c2_i_out = (((real_T)*c2_veh_spd < 0.05 * (real_T)*c2_veh_spd / 100.0)
                      && ((real_T)*c2_Decel_Cmd < 0.05) && ((real_T)
            *c2_Accel_Cmd <= 0.005));
          if (c2_i_out) {
        ...
        *c2_Mode = 3U;
        ...
        _SFD_CT_CALL(TRANSITION_BEFORE_PROCESSING_TAG, 6U,
                     chartInstance->c2_sfEvent);
        c2_j_out = (CV_TRANSITION_EVAL(6U, (int32_T)_SFD_CCP_CALL(6U, 0, (real_T)*
          c2_Accel_Cmd > 0.25 * (real_T)*c2_veh_spd / 100.0 != 0U,
          chartInstance->c2_sfEvent)) != 0);
        if (c2_j_out) {
          if (_SFD_TRANSITION_CONFLICT_CHECK_ENABLED()) {
            unsigned int transitionList[2];
            ...
            transitionList[0] = 6;
            _SFD_TRANSITION_CONFLICT_CHECK_BEGIN();
            c2_k_out = (((real_T)*c2_veh_spd < 0.05 * (real_T)*c2_veh_spd /
                         100.0) && ((real_T)*c2_Decel_Cmd < 0.05) && ((real_T)
              *c2_Accel_Cmd <= 0.005));
            if (c2_k_out) {
              transitionList[numTransitions] = 7;
              numTransitions++;
          ...
          *c2_Mode = 1U;
          ...
          _SFD_CT_CALL(TRANSITION_BEFORE_PROCESSING_TAG, 7U,
                       chartInstance->c2_sfEvent);
          c2_e_temp = (_SFD_CCP_CALL(7U, 0, (real_T)*c2_veh_spd < 0.05 * (real_T)*
            c2_veh_spd / 100.0 != 0U, chartInstance->c2_sfEvent) != 0);
          if (c2_e_temp) {
            c2_e_temp = (_SFD_CCP_CALL(7U, 1, (real_T)*c2_Decel_Cmd < 0.05 != 0U,
              chartInstance->c2_sfEvent) != 0);
          }

          c2_f_temp = c2_e_temp;
          if (c2_f_temp) {
            c2_f_temp = (_SFD_CCP_CALL(7U, 2, (real_T)*c2_Accel_Cmd <= 0.005 !=
              0U, chartInstance->c2_sfEvent) != 0);
          }

          c2_l_out = (CV_TRANSITION_EVAL(7U, (int32_T)c2_f_temp) != 0);
          if (c2_l_out) {
     ...
     case c2_IN_Decel:
      ...
      _SFD_CT_CALL(TRANSITION_BEFORE_PROCESSING_TAG, 4U,
                   chartInstance->c2_sfEvent);
      c2_m_out = (CV_TRANSITION_EVAL(4U, (int32_T)_SFD_CCP_CALL(4U, 0, (real_T)
        *c2_veh_spd < 0.05 * (real_T)*c2_veh_spd / 100.0 != 0U,
        chartInstance->c2_sfEvent)) != 0);
      if (c2_m_out) {
        if (_SFD_TRANSITION_CONFLICT_CHECK_ENABLED()) {
          unsigned int transitionList[3];
          ...
          transitionList[0] = 4;
          _SFD_TRANSITION_CONFLICT_CHECK_BEGIN();
          c2_n_out = (((real_T)*c2_Decel_Cmd < 0.05 * (real_T)*c2_veh_spd /
          ...
          c2_o_out = ((*c2_Accel_Cmd > 0.0F) && (*c2_Decel_Cmd == 0.0F));
          if (c2_o_out) {
        ...
        *c2_BrakeSet = TRUE;
        ...
        _SFD_CT_CALL(TRANSITION_BEFORE_PROCESSING_TAG, 5U,
                     chartInstance->c2_sfEvent);
        c2_g_temp = (_SFD_CCP_CALL(5U, 0, (real_T)*c2_Decel_Cmd < 0.05 * (real_T)*
          c2_veh_spd / 100.0 != 0U, chartInstance->c2_sfEvent) != 0);
        if (c2_g_temp) {
          c2_g_temp = (_SFD_CCP_CALL(5U, 1, *c2_Accel_Cmd == 0.0F != 0U,
            chartInstance->c2_sfEvent) != 0);
        }

        c2_p_out = (CV_TRANSITION_EVAL(5U, (int32_T)c2_g_temp) != 0);
        if (c2_p_out) {
          if (_SFD_TRANSITION_CONFLICT_CHECK_ENABLED()) {
            unsigned int transitionList[2];
            unsigned int numTransitions = 1;
            transitionList[0] = 5;
            _SFD_TRANSITION_CONFLICT_CHECK_BEGIN();
            c2_q_out = ((*c2_Accel_Cmd > 0.0F) && (*c2_Decel_Cmd == 0.0F));
            if (c2_q_out) {
              transitionList[numTransitions] = 10;
              numTransitions++;
            }
          ...
          *c2_Mode = 2U;
          ...
          _SFD_CT_CALL(TRANSITION_BEFORE_PROCESSING_TAG, 10U,
                       chartInstance->c2_sfEvent);
          c2_h_temp = (_SFD_CCP_CALL(10U, 0, *c2_Accel_Cmd > 0.0F != 0U,
            chartInstance->c2_sfEvent) != 0);
          if (c2_h_temp) {
            c2_h_temp = (_SFD_CCP_CALL(10U, 1, *c2_Decel_Cmd == 0.0F != 0U,
              chartInstance->c2_sfEvent) != 0);
          }

          c2_r_out = (CV_TRANSITION_EVAL(10U, (int32_T)c2_h_temp) != 0);
          if (c2_r_out) {
     ...
     case c2_IN_Stop:
      ...
      _SFD_CT_CALL(TRANSITION_BEFORE_PROCESSING_TAG, 1U,
                   chartInstance->c2_sfEvent);
      c2_s_out = (CV_TRANSITION_EVAL(1U, (int32_T)_SFD_CCP_CALL(1U, 0, (real_T)
        *c2_Accel_Cmd > 0.01 != 0U, chartInstance->c2_sfEvent)) != 0);
      if (c2_s_out) {
...
{
  ((real_T *)mxGetPr((plhs[0])))[0] = (real_T)(235323351U);
  ((real_T *)mxGetPr((plhs[0])))[1] = (real_T)(1058657706U);
  ((real_T *)mxGetPr((plhs[0])))[2] = (real_T)(1714631650U);
  ((real_T *)mxGetPr((plhs[0])))[3] = (real_T)(2445827443U);
}
...
static const mxArray *sf_get_sim_state_info_c2_EV_Pwr_Manager(void)
{
  ...
  const char *infoEncStr[] = {
    "100 S1x4'type','srcId','name','auxInfo'{{M[1],M[25],T\"BrakeSet\",},{M[1],M[22],T\"Mode\",},{M[8],M[0],T\"is_active_c2_EV_Pwr_Manager\",},{M[9],M[0],T\"is_c2_EV_Pwr_Manager\",}}"
  };
  ...
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        {
          static unsigned int sStartGuardMap[] = { 2, 35, 56 };

          static unsigned int sEndGuardMap[] = { 26, 49, 73 };
...

        {
          ...
          static unsigned int sEndGuardMap[] = { 26, 49 };
...
static void mdlSetWorkWidths_c2_EV_Pwr_Manager(SimStruct *S)
{
  ...
  ssSetChecksum0(S,(3547056618U));
  ssSetChecksum1(S,(1499573796U));
  ssSetChecksum2(S,(2553868668U));
  ssSetChecksum3(S,(2522172742U));
